...
chromSize=$(refgenie seek hg38/fasta.chrom_sizes)
...
for f in *; do
    if [ -d "$f" ]; then
        ...
        mv $f.bedGraph /Volumes/BackUp_AubleLab/ENCODE_files_cellSpecificity/hg38/primaryCells_merged_bedGraph
        mv $f.bw /Volumes/BackUp_AubleLab/ENCODE_files_cellSpecificity/hg38/primaryCells_merged_bigWig
