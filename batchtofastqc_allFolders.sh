...
#This script will map NextSeq FASTQ files in batch for one particular sample to the human hg19 genome using bowtie2, then convert the output to BAM, merge the individual files from each flow cell, filter unmapped reads, sort and index the filtered BAM file, convert to BigWig and run FastQC
#To capture the screen output in a file use this command syntax: nohup batchtofastqc_allFolders.sh > batchoutput.txt & (ampersand to run in background, leave off to run in foreground)
...
for folder in */
	do
		...
		for i in *.fastq 
			do
				echo "Processing following FASTQ file:"
 				echo $i
				bowtie2 -x /Users/lilcrusher/bowtie2-2.2.6/indexes/hg19/hg19 -p 6 --time -U $i | samtools view -S -b - > $i.bam; 
				gzip $i
		done 
		...
		bedtools intersect -abam filt.merged.sorted.bam -b /Users/lilcrusher/scripts/blacklisted_sites/sorted_consensusBlacklist.bed -v > filt.merged.sorted.bl.bam
		...
		bedtools genomecov -i filt.merged.sorted.bl.bam.bed -g ~/gentools/chromsize/hg19chrom.sizes -bg > filt.merged.sorted.bl.bam.bed.cov; bedGraphToBigWig filt.merged.sorted.bl.bam.bed.cov ~/gentools/chromsize/hg19chrom.sizes filt.merged.sorted.bl.bam.bed.bw
...
# excel table
python /Users/lilcrusher/scripts/TxtToXls_forLoop.py
