...
#to generate average plots with respect to a set of annotations (e.g. TSS), change paths to annotation file and to directory with files you want to plot.  Plots will be generated for all bigWig files in the directory 
#skip calc.highest.probe unless you want to align the profiles with the peak nearest the annotations; in general skip this first function and just align with respect to the annotation
#then run code to define window.step, composites.test,and composites.func.panels.chromatin or composites.func.panels.chromatinalt functions
#after than, run code to format annotations (tss code), then a = composites.test..., and finally composites.func.panels.chromatin... or composites.func.panels.chromatinalt... commands, which will generate a pdf in the working directory
#note that this script is the same as guertin_MNase-seq_plots.r except that it has the option to make plots without labeled gray box at top of plot (see composites.func.panels.chromatinalt below)
library(lattice)
library(bigWig)
...
#or use this version of the above function if you don't want the gray box for a label at the top of the plot
composites.func.panels.chromatinalt <- function(dat, fact = 'Factor ', summit = 'Summit', num =90) {
  col.lines = c(rgb(0 ,0 ,1 ,1/2), rgb(1, 0, 0, 1/2), "purple", "#0faa12")
  count = length(unique(dat$grp))
  pdf(paste('composite _', fact , '_signals_', summit , '_peaks.pdf', sep=''), width = 2 * 3.43, height = ceiling((count/2)) * 3.43) #change width = 2 * 3.43 to something like 1 * 3.43 to better fill page
  print(xyplot(est ~ x, group = cond, data = dat, type = 'l',
               #switch x|grp and group = cond above to get different panels as shown below as alternative
               #scales = list(x= list(cex =0.8, at=seq(-(num), (num), (num)/3), relation = "free"), y = list(cex =0.8, relation ="free")), xlim =c(-(num) ,(num)),
               col = col.lines,
               auto.key = list(points = F, lines =T, cex =0.8),
               ...
               cex.axis =1.0,
               par.strip.text = list(cex =0.9 , font =1, col ='black'),
               aspect =0.5, #set to <1 for wider plot and >1 for narrower plot
               between = list (y=0.5 , x =0.5) ,
               # index . cond = list (c(2 ,1)),
               lwd =3,
               ylab = list(label = paste(fact," MNase signal", sep=''), cex =0.8),
...
tss = read.table('/Users/dauble/data/yeast_ChIP-seq_data_2015-2016/MNase-seq/Guertin_+1_nucleosome_alignment/data_files/bw/from_dpos/reanalysis_with_renorm_files/annotations/filtered_annotation_results_down_header.txt', stringsAsFactors=FALSE) #change path as needed
tss[,6][tss[,6] == 1] = '+'
tss[,6][tss[,6] == -1] = '-'
...
a = composites.test('/Users/dauble/data/yeast_ChIP-seq_data_2015-2016/MNase-seq/Guertin_+1_nucleosome_alignment/data_files/bw/from_dpos/reanalysis_with_renorm_files/', tss, region =500, step =1) #change path as needed & region = distance in bp around annotation to plot
...
#use this next version of the above for a plot with no labeled box at top
composites.func.panels.chromatinalt(a[[1]], fact = 'Average', summit = 'TSS', num =800)
