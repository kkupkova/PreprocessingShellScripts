# use this script for computing background in "sparse" data such as H3K4me3 data in human cells; this script uses all the data including the (many) bins with zero reads.  Compare the results using the script Fit_NonZeroBkgrdRC_Exp.R which excludes the zero bins.  
#this script is identical to Fit_BkgrdRC_Exp.R except that it provides the adjusted R^2 value at the end.
#To begin, set working directory to location of files, then use the following to specify which file to analyze.
rc_filename <- "binned_maternal150_1409.txt"
#in the above, my_file.txt is in 3 column BED format and column 4 is the number of reads in each bin
#after that, run the script beginning at line 11
#(the following two lines are for running this script on the command line)
args <- commandArgs(TRUE)
rc_filename <- args[1]
#as stated above, if specifying rc_filename <- "my_file" from the working directory, then run script beginning at line 11.
