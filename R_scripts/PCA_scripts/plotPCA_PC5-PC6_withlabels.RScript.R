#use this script to plot PC5 versus PC6 with sample names
...
plotPCAPC56WithSampleNames = function(x, intgroup="condition", ntop=500)
{
  ...
  #plot parameters
  xyplot(
    PC6 ~ PC5, groups=fac, data=as.data.frame(pca$x), pch=16, cex=1.5,
    panel=function(x, y, ...) {
      panel.xyplot(x, y, ...);
...
print(plotPCAPC56WithSampleNames(rld,intgroup=c("condition")))

#or this command, as appropriate 
print(plotPCAPC56WithSampleNames(vsd,intgroup=c("condition")))
