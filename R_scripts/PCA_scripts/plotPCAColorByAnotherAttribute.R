...
plotPCAWithGradient = function(x, intgroup="condition", ntop=500)
{
  ...
  names = colnames(x)
  #if you want to plot with different colors based on an attribute like age, use something like this
  age <<- samples_correlation$age
  
  #and similarly if you want the shape based on some other attribute
  condition <<- samples_correlation$condition
  ...
  #plot parameters- change age in next command to name of biomarker above
  #change PC1 and PC2 to generate plots of other principal components
  pca1<-ggplot(data=as.data.frame(pca$x),aes(PC1,PC2,colour=age))
  ...
  #The following sets the color and the datapoint shape
  pca2<-pca1 + geom_point(aes(shape=condition),size=4) +scale_color_manual(values=c("dodgerblue4","goldenrod1","darkorchid3")) + scale_shape_manual(values=c(16,15)) 
  
  #to hide the shape legend use
  #+ scale_shape_manual(values=c(16,15),guide=FALSE) 
  
  #other color option (2 colors)
  #+scale_colour_manual(values=c("dodgerblue","firebrick3"))
  
  #for no condition differences use this
  pca2<-pca1 + geom_point(size=4) +scale_color_manual(values=c("dodgerblue4","goldenrod1","darkorchid3"))
  
  #note more nice colors
  #colours = c( "darkgreen", "dodgerblue", "firebrick3","darkgoldenrod1","darkturquoise" )
