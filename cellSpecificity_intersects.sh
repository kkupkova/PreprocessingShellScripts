...
# Run from a folder with the bedGraph files - the output folders will 
...
for bedFile in "$bedFolder"/*.bed
 ...
 # 
  ...
  mkdir $folderReference
	...
	# go through the bedGraph files and intersect them with the BED file
	...
	for i in *.bedGraph
	 do 
		...
		# -wao will keep both bed and bedGraph co-rdinates and size of overlap
		bedtools intersect -a $bedFile -b $i -wao > $output
